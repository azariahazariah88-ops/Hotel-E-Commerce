...
function App() {
  ...
  return (
    <div style={{ fontFamily: "'Lora', serif", background: "#0d0d0d", minHeight: "100vh" }}>
      ...
      {/* ── Hero ─────────────────────────────────────────────────── */}
      <div style={{ position: "relative", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "linear-gradient(135deg, #0d0d0d 0%, #1a1200 50%, #0d0d0d 100%)", overflow: "hidden" }}>

        {/* Glow orbs */}
        <div style={{ position: "absolute", width: 700, height: 700, borderRadius: "50%", background: "radial-gradient(circle, rgba(245,200,66,0.07) 0%, transparent 70%)", top: "50%", left: "50%", transform: "translate(-50%,-50%)", animation: "glow 5s ease-in-out infinite" }} />
        <div style={{ position: "absolute", width: 350, height: 350, borderRadius: "50%", background: "radial-gradient(circle, rgba(245,200,66,0.05) 0%, transparent 70%)", top: "15%", left: "8%", animation: "glow 7s ease-in-out infinite 1s" }} />
        <div style={{ position: "absolute", width: 250, height: 250, borderRadius: "50%", background: "radial-gradient(circle, rgba(245,200,66,0.04) 0%, transparent 70%)", bottom: "15%", right: "10%", animation: "glow 6s ease-in-out infinite 2s" }} />
        <style>{`@keyframes glow { 0%,100%{opacity:0.6;transform:scale(1)} 50%{opacity:1;transform:scale(1.1)} }`}</style>
        ...
        <div style={{ position: "relative", zIndex: 2, textAlign: "center", padding: "2rem 1rem", maxWidth: 700, margin: "0 auto", width: "100%" }}>
          ...
          <div style={{ position: "relative", maxWidth: 520, margin: "0 auto" }}>
            <div style={{ position: "relative" }}>
              <i className="bi bi-search" style={{ position: "absolute", left: 18, top: "50%", transform: "translateY(-50%)", color: "#f5c842", fontSize: "1rem", zIndex: 2, pointerEvents: "none" }} />
              <input
                ref={inputRef}
                type="text"
                value={query}
                ...
                style={{ width: "100%", padding: "14px 48px 14px 46px", borderRadius: showDrop && results.length > 0 ? "20px 20px 0 0" : 50, border: "2px solid rgba(245,200,66,0.45)", background: "rgba(10,10,10,0.88)", backdropFilter: "blur(12px)", color: "#fff", fontSize: "0.95rem", outline: "none", boxSizing: "border-box", transition: "border-radius 0.2s", fontFamily: "'Lora', serif" }}
              />
              {query && (
                <button onClick={() => { setQuery(""); setResults([]); setShowDrop(false); inputRef.current?.focus() }}
                  style={{ position: "absolute", right: 14, top: "50%", transform: "translateY(-50%)", background: "transparent", border: "none", color: "#666", cursor: "pointer", fontSize: "1rem", lineHeight: 1, padding: 4 }}>✕</button>
              )}
            </div>

            {/* Dropdown */}
            {showDrop && (
              <div ref={dropRef} style={{ position: "absolute", top: "100%", left: 0, right: 0, background: "rgba(12,12,12,0.98)", backdropFilter: "blur(20px)", border: "2px solid rgba(245,200,66,0.35)", borderTop: "none", borderRadius: "0 0 20px 20px", maxHeight: 380, overflowY: "auto", zIndex: 100, textAlign: "left" }}>
                {results.length === 0 ? (
                  ...
                  <>
                    {["food", "grocery", "cake"].map((cat) => {
                      ...
                      return (
                        <div key={cat}>
                          <div style={{ padding: "8px 18px 4px", fontSize: "0.7rem", letterSpacing: 3, textTransform: "uppercase", color: "#f5c842", borderTop: "1px solid rgba(255,255,255,0.05)", background: "rgba(245,200,66,0.04)" }}>
                            {CATEGORY_EMOJI[cat]} {cat}
                          </div>
                          {items.map((item) => {
                            const isAdded = addedId === item.id
                            return (
                              <div key={item.id} onClick={() => handleGoToCategory(item)}
                                style={{ display: "flex", alignItems: "center", gap: 14, padding: "10px 18px", cursor: "pointer", transition: "background 0.15s" }}
                                onMouseEnter={(e) => (e.currentTarget.style.background = "rgba(245,200,66,0.07)")}
                                onMouseLeave={(e) => (e.currentTarget.style.background = "transparent")}
                              >
                                <img src={item.image} alt={item.name} style={{ width: 46, height: 46, objectFit: "cover", borderRadius: 10, flexShrink: 0 }} />
                                <div style={{ flex: 1, minWidth: 0 }}>
                                  <div style={{ color: "#fff", fontSize: "0.92rem", fontWeight: 500 }}><Highlight text={item.name} query={query} /></div>
                                  <div style={{ color: "#f5c842", fontSize: "0.82rem", marginTop: 2 }}>₹{item.price}</div>
                                </div>
                                <button onClick={(e) => handleAddToCart(item, e)}
                                  style={{ background: isAdded ? "#28a745" : "rgba(245,200,66,0.12)", color: isAdded ? "#fff" : "#f5c842", border: `1px solid ${isAdded ? "#28a745" : "rgba(245,200,66,0.35)"}`, borderRadius: 20, padding: "5px 14px", fontSize: "0.78rem", fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap", transition: "all 0.25s", fontFamily: "'Lora', serif", flexShrink: 0 }}>
                                  {isAdded ? "✓ Added" : "+ Cart"}
                                </button>
                              </div>
                            )
                    ...
                    <div style={{ padding: "10px 18px", fontSize: "0.73rem", color: "#3a3a3a", borderTop: "1px solid rgba(255,255,255,0.04)", textAlign: "center" }}>
          ...
          <div className="row justify-content-center">
            {categories.map((cat, i) => (
              <div className="col-md-4 col-sm-6 mb-4" key={i}>
                <div onClick={() => navigate(cat.path)}
                  style={{ background: "#1a1a1a", border: "1px solid #2a2a2a", borderRadius: 20, overflow: "hidden", cursor: "pointer", transition: "all 0.35s cubic-bezier(0.34,1.56,0.64,1)" }}
                  onMouseEnter={(e) => { e.currentTarget.style.transform = "translateY(-10px) scale(1.02)"; e.currentTarget.style.border = "1px solid rgba(245,200,66,0.5)"; e.currentTarget.style.boxShadow = "0 20px 50px rgba(245,200,66,0.12)" }}
                  onMouseLeave={(e) => { e.currentTarget.style.transform = "none"; e.currentTarget.style.border = "1px solid #2a2a2a"; e.currentTarget.style.boxShadow = "none" }}>
                  ...
                  <div style={{ padding: "20px 22px 24px" }}>
                    ...
                    <span style={{ color: "#f5c842", fontSize: "0.85rem", letterSpacing: 1 }}>Order now <i className="bi bi-arrow-right ms-1" /></span>
                  </div>
                </div>
              </div>
            ))}
